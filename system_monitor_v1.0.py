...
También puede descargar y ejecutar archivos desde el servidor.
"""
...
SSH_CONFIG = {
    ...
    'remote_scripts_dir': '/root/scripts'  # 🔧 Directorio remoto para scripts
}
...
class SSHFileUploader:
    ...
    def download_file(self, remote_filename, local_filename=None):
        ...
        Args:
            ...
            local_filename (str, optional): Nombre del archivo local
        ...
        remote_path = f"{SSH_CONFIG['remote_scripts_dir']}/{remote_filename}"
        ...
        try:
            # 2. Verificar si el archivo remoto existe
            try:
    ...
    def list_remote_files(self, remote_dir=None):
        """
        Lista archivos disponibles en el directorio remoto

        Args:
            remote_dir (str, optional): Directorio remoto a listar

        Returns:
            list: Lista de archivos disponibles
        """
        if not remote_dir:
            remote_dir = SSH_CONFIG['remote_scripts_dir']
    ...
    def execute_downloaded_file(self, file_path, args=None):
        """
        Ejecuta un archivo descargado

        Args:
            file_path (str): Ruta del archivo a ejecutar
            args (list, optional): Argumentos para el archivo
        ...
        try:
            # Determinar el tipo de archivo y ejecutarlo
            ...
            if file_extension == '.py':
                ...
                # Intentar ejecutar como ejecutable genérico
                cmd = [file_path]
            ...
            return {"success": False, "error": "Tiempo de ejecución agotado"}
        except Exception as e:
            return {"success": False, "error": str(e)}

    def download_and_execute(self, remote_filename, args=None):
        """
        Descarga un archivo desde el servidor y lo ejecuta automáticamente

        Args:
            remote_filename (str): Nombre del archivo remoto a descargar y ejecutar
            args (list, optional): Argumentos para el archivo
        ...
        local_path = self.download_file(remote_filename)

        if not local_path:
            return {
                "success": False,
                "error": f"No se pudo descargar el archivo {remote_filename}"
            }

        # 2. Ejecutar el archivo
        execution_result = self.execute_downloaded_file(local_path, args)

        # 3. Limpiar archivo temporal (opcional)
        try:
...
def main():
    ...
    # 5. Ejemplo: Descargar y ejecutar un archivo desde el servidor
    # Descomenta las siguientes líneas para usar esta funcionalidad:
    #
    # # Listar archivos disponibles en el servidor
    # available_files = uploader.list_remote_files()
    # print(f"Archivos disponibles en el servidor: {available_files}")
    #
    # # Descargar y ejecutar un archivo específico
    # if available_files:
    #     result = uploader.download_and_execute(available_files[0])
    #     if result['success']:
    #         print(f"Archivo ejecutado exitosamente: {result['execution_result']['stdout']}")
    #     else:
    #         print(f"Error ejecutando archivo: {result['error']}")
